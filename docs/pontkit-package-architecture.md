...
| Versioning | Every renamed package **starts at `1.0.0`**, not at the next major of the package it replaces. | Decided 2026-09-06, reversing an earlier `3.0.0`. That number came from continuing `@saasbase-io/core-elements@2.16.x` and `@saasbase-io/elements@2.10.x`, but a new package name has no version history to continue — npm sees `@pontive/pontkit-core` for the first time, and `3.0.0` would advertise two majors of a lineage nobody can install. `@pontive/oidc-client` already shipped `1.0.0` on the same reasoning. The old packages are deprecated, not superseded in-place, so nothing resolves across the boundary and no range is ambiguous. |
...
**A version range is forbidden, not merely unnecessary.** An exact release is immutable on the CDN, cacheable forever, and is the only form that admits an SRI hash — `pontive-spec/21-element-versioning-and-compatibility.md` §5 makes this normative. The major-range URL the first draft proposed would silently drop integrity checking.
