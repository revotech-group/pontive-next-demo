...
- **Ingress rides the existing shared ALB.** `ingressClassName: alb-shared`, the class `pontive-gitops/overlays/dev/ingress.yaml` uses for the gRPC hosts. `alb-pontive` is the services' own class and carries their `IngressClassParams` group; the demos do not join it. The cluster runs the EKS Auto Mode ALB controller (`controller: eks.amazonaws.com/alb`), so there is no plain `alb` class to reference.
- **One Ingress for every demo, not one per demo — and this is a cost decision, not a tidiness one.** The ALB controller decides how many load balancers to provision from how Ingresses are grouped, and the grouping for `alb-shared` lives in an `IngressClassParams` that is in neither `pontive-gitops` nor `platform-gitops`. An Ingress per demo therefore *might* have produced an ALB per demo at roughly $20/month each — which is about what Vercel Pro costs, and would have quietly dismantled the "~$0 marginal, flat in N" argument that chose EKS in §3. A single Ingress carrying a host rule per demo makes the outcome independent of a grouping rule nobody in this design can see. It is also the shape `pontive-gitops` already uses: one Ingress, two gRPC hosts.
